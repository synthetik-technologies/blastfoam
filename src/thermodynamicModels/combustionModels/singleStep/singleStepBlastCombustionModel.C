...
#include "singleStepBlastCombustionModel.H"
#include "fvmSup.H"


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::combustionModels::singleStep::calculateqFuel()
{
...
void Foam::combustionModels::singleStep::massAndAirStoichRatios()
{
...
void Foam::combustionModels::singleStep::calculateMaxProducts()
{
    scalar Wm = 0.0;
    scalar totalMol = 0.0;
    forAll(reaction_.rhs(), i)
...
Foam::combustionModels::singleStep::singleStep
(
...
:
    blastCombustionModel(modelType, thermo, combustionProperties),
    reaction_
    (
        this->coeffs().isDict("reaction")
      ? reaction(thermo_.species(), this->coeffs().subDict("reaction"))
      : reaction(thermo_.species(), this->coeffs().lookup<string>("reaction"))
    ),
    ...
    fres_(Yprod0_.size()),
    fuelIndex_(thermo_.species()[coeffs_.lookup("fuel")]),
    ...
    semiImplicit_(readBool(this->coeffs().lookup("semiImplicit")))
{
    forAll(fres_, fresI)
    {
        IOobject header
...
Foam::combustionModels::singleStep::R(const label speciei) const
{
...
Foam::combustionModels::singleStep::R(volScalarField& Y) const
{
...
Foam::combustionModels::singleStep::Qdot() const
{
...
bool Foam::combustionModels::singleStep::read()
{
...
void Foam::combustionModels::singleStep::fresCorrect()
{
    const label O2Index = thermo_.species()["O2"];
