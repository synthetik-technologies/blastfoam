...
)
{
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        ...
        thisCmpts.append
        (
            basicThermoModel::splitThermoName(ThermoType::typeName(), 5)
        );

        wordList validNames;
        forAll(names, i)
        {
            const wordList cmpts(basicThermoModel::splitThermoName(names[i], 7));
        ...
        forAll(names, i)
        {
            validCmpts.append(basicThermoModel::splitThermoName(names[i], 7));
        }
