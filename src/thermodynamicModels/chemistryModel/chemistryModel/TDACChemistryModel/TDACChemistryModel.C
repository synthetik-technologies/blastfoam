...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("TabulationResults", thermo.name()),
            this->time().timeName(),
            this->mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
...
) const
{
    ...
    forAll(this->reactions_, i)
    {
        if (!reactionsDisabled_[i])
        {
            ...
            (
                p, this->rhoi_, T, c, pf, cf, lRef, pr, cr, rRef
            );
...
{
    const scalar kf = R.kf(p, this->rhoi_, T, c);
    const scalar kr = R.kr(kf, p, this->rhoi_, T, c);
...
) const
{
    ...
    for (label i=0; i<this->c_.size(); i++)
    {
        ...
        cp += this->c_[i]*this->specieThermo_[i].cp(rho, e, T);
    }
    cp /= rho;
    ...
    for (label i=0; i<this->nSpecie_; i++)
    {
        ...
        const scalar hi = this->specieThermo_[si].ha(p, rho, T);
        dT += hi*dcdt[i];
    }
    dT /= rho*cp;
...
) const
{
    ...
    forAll(hi, i)
    {
        hi[i] = this->specieThermo_[i].ha(p, rho, T);
        cpi[i] = this->specieThermo_[i].cp(p, rho, T);
    }
...
)
{
    ...
    forAll(this->thermo().rho(), celli)
    {
        ...
        for (label i=0; i<this->nSpecie_; i++)
        {
            c[i] = this->rhoi_*this->Y_[i][celli]/this->specieThermo_[i].W();
            c0[i] = c[i];
        ...
        if (tabulation_->active() && tabulation_->retrieve(phiq, Rphiq))
        {
            ...
            for (label i=0; i<this->nSpecie(); i++)
            {
                c[i] = this->rhoi_*Rphiq[i]/this->specieThermo_[i].W();
            }
        ...
        else
        {
            ...
            if (tabulation_->active())
            {
                forAll(c, i)
                {
                    Rphiq[i] = c[i]/this->rhoi_*this->specieThermo_[i].W();
                }
                if (tabulation_->variableTimeStep())
                {
                ...
                label growOrAdd =
                    tabulation_->add(phiq, Rphiq, this->rhoi_, deltaT[celli]);
                if (growOrAdd)
                {
    ...
    if (Pstream::parRun())
    {
        List<bool> active(this->thermo().active());
