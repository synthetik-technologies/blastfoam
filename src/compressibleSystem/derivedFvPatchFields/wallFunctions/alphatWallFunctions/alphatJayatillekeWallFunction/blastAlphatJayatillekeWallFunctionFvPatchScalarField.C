...
#include "blastAlphatJayatillekeWallFunctionFvPatchScalarField.H"
...
void alphatJayatillekeWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    const fvPatchScalarField& ew =
        turbModel.transport().e().boundaryField()[patchi];

    // Heat flux [W/m^2] - lagging alphatw
    const scalarField qDot
    (
        turbModel.transport().alphaEff(alphatw, patchi)*ew.snGrad()
    );
