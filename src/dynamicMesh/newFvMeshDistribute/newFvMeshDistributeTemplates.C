...
void Foam::newFvMeshDistribute::printFieldInfo(const fvMesh& mesh)
{
...
void Foam::newFvMeshDistribute::saveBoundaryFields
(
...
template<class T, class Mesh>
void Foam::newFvMeshDistribute::mapBoundaryFields
(
...
template<class T>
void Foam::newFvMeshDistribute::saveInternalFields
(
...
template<class T>
void Foam::newFvMeshDistribute::mapExposedFaces
(
    const mapPolyMesh& map,
...
)
{
    ...
    {
        FatalErrorIn("newFvMeshDistribute::mapExposedFaces(..)") << "problem"
            << abort(FatalError);
    }
    ...
    forAllIter(typename HashTable<fldType*>, flds, iter)
    {
        ...
        {
            fvsPatchField<T>& patchFld = bfld[patchi];
            // Pout<<"post: "<<fld.name()<<" "<<mesh_.boundary()[patchi].name()<<" "<<patchFld.patch().start()+patchFld.size()<<" "<<mesh_.nInternalFaces()<<endl;

            forAll(patchFld, i)
            {
                const label facei = patchFld.patch().start()+i;
                // if (facei < faceMap.size())
                {
...
template<class GeoField>
void Foam::newFvMeshDistribute::correctProcessorPatchFields()
{
...
template<class GeoField>
void Foam::newFvMeshDistribute::sendFields
(
    const label domain,
    const wordList& fieldNames,
    const newFvMeshSubset& subsetter,
    Ostream& toNbr
)
{
...
template<class GeoField>
void Foam::newFvMeshDistribute::receiveFields
(
    const label domain,
    const wordList& fieldNames,
