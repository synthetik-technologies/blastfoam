...
typename Table::iterator Foam::simpleEOS::lookupThermo
(
    Table* tablePtr,
...
)
{
    ...
    if (cstrIter == tablePtr->end())
    {
        FatalErrorInFunction
            << "Unknown " << simpleEOS::typeName << " type " << nl
            << "thermoType " << thermoTypeName << nl << nl
            << "Valid " << simpleEOS::typeName << " types are:"
            << nl << nl;
        ...
        validThermoTypeNameCmpts[0].setSize(4);
        ...
        forAll(validThermoTypeNames, i)
        {
            wordList names
            (
                simpleEOS::splitThermoName(validThermoTypeNames[i])
            );
            if (names.size())
            {
