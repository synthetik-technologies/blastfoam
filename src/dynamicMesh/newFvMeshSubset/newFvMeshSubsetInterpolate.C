...
Foam::newFvMeshSubset::interpolate
(
...
)
{
    ...
    forAll(bf, patchi)
    {
        if (patchMap[patchi] != -1)
        {
            ...
            if (min(directAddressing) < 0)
            {
                Field<Type> pI(bf[patchi].patchInternalField());
                forAll(bf[patchi], i)
                {
                    if (directAddressing[i] < 0)
                    {
                        bf[patchi][i] = pI[i];
                    }
                }
...
)
{
    ...
    forAll(bf, patchi)
    {
        ...
        if (patchMap[patchi] != -1)
        {
            ...
            if
            (
                min(directAddressing) < 0
             && isA<valuePointPatchField<Type>>(bf[patchi])
            )
            {
                valuePointPatchField<Type>& vppf =
                    dynamicCast<valuePointPatchField<Type>>(bf[patchi]);
