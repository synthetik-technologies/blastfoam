...
template<class Type>
void Foam::newFvMeshAdder::MapVolField
(
...

    {
        ...
        forAll(oldPatchMap, patchi)
        {
            ...
            if (newPatchi != -1)
            {
                ...
                if (patchMapper.hasUnmapped())
                {
                    Field<Type> pI(bfld[patchi].patchInternalField());
                    forAll(bfld[patchi], i)
                    {
                        if (newToOld[i] < 0)
                        {
                            bfld[patchi][i] = pI[i];
...
template<class Type>
void Foam::newFvMeshAdder::MapVolFields
(
    const mapAddedPolyMesh& meshMap,
    const fvMesh& mesh,
...
template<class Type>
void Foam::newFvMeshAdder::MapSurfaceField
(
    const mapAddedPolyMesh& meshMap,
...
template<class Type>
void Foam::newFvMeshAdder::MapSurfaceFields
(
    const mapAddedPolyMesh& meshMap,
    const fvMesh& mesh,
    const fvMesh& meshToAdd
...
template<class Type>
void Foam::newFvMeshAdder::MapPointField
(
...

    {
        ...
        forAll(oldPatchMap, patchi)
        {
            ...
            if (newPatchi != -1)
            {
                ...
                if
                (
                    patchMapper.hasUnmapped()
                 && isA<valuePointPatchField<Type>>(bfld[patchi])
                )
                {
                    valuePointPatchField<Type>& vppf =
                        dynamicCast<valuePointPatchField<Type>>(bfld[patchi]);
                    Field<Type> pI(vppf.patchInternalField());
                    forAll(vppf, i)
                    {
                        if (newToOld[i] < 0)
                        {
...
template<class Type>
void Foam::newFvMeshAdder::MapPointFields
(
    const mapAddedPolyMesh& meshMap,
    const pointMesh& mesh,
    const labelListList& oldMeshPoints,
...
template<class Type>
void Foam::newFvMeshAdder::MapDimField
(
    const mapAddedPolyMesh& meshMap,
...
template<class Type>
void Foam::newFvMeshAdder::MapDimFields
(
    const mapAddedPolyMesh& meshMap,
    const fvMesh& mesh,
    const fvMesh& meshToAdd
)
{
    ...
    )
    {
        ...
        {
            WarningIn("newFvMeshAdder::MapDimFields(..)")
                << "Not mapping field " << fld.name()
                << " since not present on mesh to add"
                << endl;
        }
