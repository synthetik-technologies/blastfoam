...
#include "burstSlipFvPatchField.H"
...
template<class Type>
Foam::burstSlipFvPatchField<Type>::burstSlipFvPatchField
(
    const fvPatch& p,
...
Foam::burstSlipFvPatchField<Type>::burstSlipFvPatchField
(
    const burstSlipFvPatchField<Type>& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
template<class Type>
void Foam::burstSlipFvPatchField<Type>::autoMap
(
...
template<class Type>
void Foam::burstSlipFvPatchField<Type>::rmap
(
...
)
{
    ...
    const burstSlipFvPatchField<Type>& bbptf =
        refCast<const burstSlipFvPatchField<Type>>(ptf);
...
Foam::burstSlipFvPatchField<Type>::truePatchNeighbourField() const
{
...
template<class Type>
Foam::tmp<Foam::Field<Type>>
Foam::burstSlipFvPatchField<Type>::patchNeighbourField() const
{
    return intact_*(*this) + (1.0 - intact_)*truePatchNeighbourField();
}


template<class Type>
void Foam::burstSlipFvPatchField<Type>::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    )
    {
        ...
        burstSlipFvPatchField<Type>& nbr =
            dynamicCast<burstSlipFvPatchField<Type>>
            (
    ...
    Field<Type> slipVals
    (
        (iF + transform(tensor::I - 2.0*sqr(nHat), iF))/2.0
    );
    Field<Type> intVals
    (
    ...
    Field<Type>::operator=
    (
        intact_*slipVals + (1.0 - intact_)*intVals
    );
}
