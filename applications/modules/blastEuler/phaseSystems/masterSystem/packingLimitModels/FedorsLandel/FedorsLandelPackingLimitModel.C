...
#include "FedorsLandelPackingLimitModel.H"
...
{
    if (system.phases().size() != 2)
    {
        FatalIOErrorInFunction(dict)
            << typeName << " packing limit model only supports bi-disperse (2)"
            << "particle distributions, but " << system.phases().size()
            << " are in use." << endl
            << exit(FatalIOError);
    }
}
