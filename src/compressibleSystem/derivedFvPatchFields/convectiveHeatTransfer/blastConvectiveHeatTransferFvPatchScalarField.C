...
#include "blastConvectiveHeatTransferFvPatchScalarField.H"
...
void convectiveHeatTransferFvPatchScalarField::updateCoeffs()
{
    ...
    const scalarField& ew = turbModel.transport().e().boundaryField()[patchi];
    const scalarField Cpw(turbModel.transport().Cp(rhow, ew, Tw, patchi));
