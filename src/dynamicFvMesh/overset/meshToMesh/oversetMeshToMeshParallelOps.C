...
Foam::label Foam::oversetMeshToMesh::calcDistribution
(
    const polyMesh& src,
    const polyMesh& tgt
) const
{
    ...
    if (Pstream::parRun())
    {
        ...
        {
            proci = findIndex(cellsPresentOnProc, 1);//.find(1);
...
Foam::label Foam::oversetMeshToMesh::calcOverlappingProcs
(
...
Foam::autoPtr<Foam::mapDistribute> Foam::oversetMeshToMesh::calcProcMap
(
    const polyMesh& src,
    const polyMesh& tgt
) const
{
    switch (procMapMethod_)
    {
        ...
        {
            Info<< "oversetMeshToMesh: Using processorLOD method" << endl;
...
void Foam::oversetMeshToMesh::distributeCells
(
...
void Foam::oversetMeshToMesh::distributeAndMergeCells
(
    const mapDistribute& map,
    const polyMesh& tgt,
    const globalIndex& globalI,
