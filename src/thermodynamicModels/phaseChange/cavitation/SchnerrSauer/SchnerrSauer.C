...
:
    ...
    (
        "residualRho",
        max(thermo1.residualRho(), thermo2.residualRho())
    ),
    minRhol_("minRhol", dimDensity, dict_)
{
    if (dict_.found("alphaNuc"))
    {
        alphaNuc_ = dict_.lookup<scalar>("alphaNuc");
    }
    else
    {
...
:
    ...
    (
        "residualRho",
        dimDensity,
        dict_.lookupOrDefault("residualRho", 1e-6)
    ),
    minRhol_
    (
        "minRhol",
        dimDensity,
        dict_.lookupOrDefault("minRhol", residualRho_.value())
    )
{
    if (dict_.found("alphaNuc"))
    {
...
Foam::phaseChangeModels::SchnerrSauer::mDotCV() const
{
    ...
    (
        max(alphav*rhov + alphal*rhol, residualRho_)
    );

    const volScalarField::Internal pSatv(this->pSatv());
    const volScalarField::Internal pSatl(this->pSatl());

    const volScalarField::Internal pCoeffv
    (
        "pCoeffv",
        3.0*rhol*rhov*rRb(alphal)
       *sqrt((2.0/3.0)/max(rhol, minRhol_))
       /(rho*sqrt(mag(p - pSatv) + 0.01*pSatv))
    );
    const volScalarField::Internal pCoeffl
    (
        "pCoeffl",
        3.0*rhol*rhov*rRb(alphal)
       *sqrt((2.0/3.0)/max(rhol, minRhol_))
       /(rho*sqrt(mag(p - pSatl) + 0.01*pSatl))
    );

    return Pair<tmp<volScalarField::Internal>>
    (
        Cc_*alphal*alphav*pCoeffv*max(p - pSatv, zeroP),
      - Cv_*alphal*(alphav + alphaNuc_)*pCoeffl*min(p - pSatl, zeroP)
    );
}


void Foam::phaseChangeModels::SchnerrSauer::correct()
{
