...
11-08-2022 Synthetik Applied Technologies : Reload processor meshes on topo
                                            changes
17-08-2022 Synthetik Applied Technologies : Added reconstructParMesh functionality
-------------------------------------------------------------------------------
License
...
Description
    ...
    execution of OpenFOAM. Include reconstruction of mesh and fields

    Includes functionality from reconstructParMesh and reconstructPar with
    support for topological changes and blastfoam refinement histories
...
int main(int argc, char *argv[])
{
    argList::addNote
    (
        "Reconstruct mesh and fields of a parallel case"
    );
...
)
{
    ...
    const word regionDir =
      ...
      ? word::null
      : regionName;
    const label nProcs = databases.size();
    ...
    //          typeHeaderOk
    if (!facesIO.typeHeaderOk<faceCompactIOList>(false) && procMeshesPtr.valid())
    {
...

    {
        ...
        PtrList<fvMesh> procMeshes(databases.size());
        ...
        for (label step=2; step<nProcs*2; step*=2)
        {
            for (label proci=0; proci<nProcs; proci+=step)
            {
                ...
                // Processors that were already in procMeshes
        ...
        masterInternalFaces = procMeshes[0].nInternalFaces();
        masterOwner = procMeshes[0].faceOwner();
    ...
    mesh.readUpdate();
    procMeshesPtr.reset(new processorMeshes(databases, regionName));
}
