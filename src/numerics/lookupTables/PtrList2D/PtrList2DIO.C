...
)
{
    ...
    if (L.size() > 1)
    {
        ...
        forAll(L, i)
        {
            if (L.PtrList<Type>::operator[](i) != L.PtrList<Type>::operator[](0))
            {
                uniform = false;
                break;
            }
    ...
    os.check("Ostream& operator<<(Ostream&, const PtrList2D&)");
