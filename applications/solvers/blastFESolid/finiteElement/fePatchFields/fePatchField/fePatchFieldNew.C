...
template<class Type>
Foam::autoPtr<Foam::fePatchField<Type>> Foam::fePatchField<Type>::New
(
...
)
{
    ...
    typename fePatchConstructorTable::iterator cstrIter =
        fePatchConstructorTablePtr_->find(patchFieldType);

    if (cstrIter == fePatchConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown patchField type "
            << patchFieldType << nl << nl
            << "Valid patchField types are :" << endl
            << fePatchConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }
    ...
    )
    {
        typename fePatchConstructorTable::iterator patchTypeCstrIter =
            fePatchConstructorTablePtr_->find(p.type());
...
)
{
    ...
    {
        if (!disallowGenericFePatchField)
        {
...
(
    ...
    const fePatch& p,
    const DimensionedField<Type, feMesh>& iF,
    const fePatchFieldMapper& pfMapper
)
{
    if (debug)
    {
        InfoInFunction << "Constructing fePatchField<Type>" << endl;
    }
