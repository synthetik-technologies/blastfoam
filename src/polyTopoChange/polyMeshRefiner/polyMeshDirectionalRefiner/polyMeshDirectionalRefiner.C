...
namespace Foam
{
    defineTypeNameAndDebug(polyMeshDirectionalRefiner, 0);
    addToRunTimeSelectionTable(polyMeshRefiner, polyMeshDirectionalRefiner, polyMesh);
    addToRunTimeSelectionTable(polyMeshRefiner, polyMeshDirectionalRefiner, dictionary);
}


void Foam::polyMeshDirectionalRefiner::addCells
(
...
void Foam::polyMeshDirectionalRefiner::update
(
    const Map<label>& splitMap,
    vectorField& field
)
...
Foam::vector Foam::polyMeshDirectionalRefiner::getDirection(const word& d)
{
    if (d == "x")
    {
        return vector(1, 0, 0);
...
Foam::labelListList Foam::polyMeshDirectionalRefiner::refineDirections
(
...
)
{
    ...
    forAll(cellDirections, dirI)
    {
        ...
        if (dirField.size() == 1)
        {
            ...
            if (debug)
            {
                Pout<< "polyMeshDirectionalRefiner : Uniform refinement:"
                    << dirField[0] << endl;
            }
        ...
        if (debug)
        {
            Pout<< "polyMeshDirectionalRefiner : Done refining direction " << dirI
...
Foam::labelListList
Foam::polyMeshDirectionalRefiner::refine(const labelList& origCellsToRefine)
{
...
Foam::polyMeshDirectionalRefiner::polyMeshDirectionalRefiner(polyMesh& mesh)
:
...
Foam::polyMeshDirectionalRefiner::polyMeshDirectionalRefiner
(
    polyMesh& mesh,
...
void Foam::polyMeshDirectionalRefiner::readDict(const dictionary& dict)
{
    polyMeshRefiner::readDict(dict);
...
bool Foam::polyMeshDirectionalRefiner::refine
(
