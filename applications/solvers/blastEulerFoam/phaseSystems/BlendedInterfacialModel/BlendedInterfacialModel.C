...
) const
{
    ...
    if (model_.valid() || model1In2_.valid())
    {
        f1 = blending_.f1i(celli, phase1_, phase2_);
    }

    if (model_.valid() || model2In1_.valid())
    {
        f2 = blending_.f2i(celli, phase1_, phase2_);
    }
    ...
    if (model_.valid())
    {
        ...
        x += (model_().*method)(celli, args ...)*(scalar(1) - f1 - f2);
    }

    if (model1In2_.valid())
    {
        x += (model1In2_().*method)(celli, args ...)*f1;
    }

    if (model2In1_.valid())
    {
        Type dx = (model2In1_().*method)(celli, args ...)*f2;
...
{
    return evaluate(&ModelType::Ki, "Ki", false, celli, nodei, nodej);
}
...
{
    return evaluate(&ModelType::F, "F", ModelType::dimF, true, nodei, nodej);
}
...
{
    return evaluate(&ModelType::Fi, "Fi", true, celli, nodei, nodej);
}
...
{
    return evaluate(&ModelType::Ff, "Ff", ModelType::dimF*dimArea, true, nodei, nodej);
}
...
{
    return evaluate(&ModelType::Di, "Di", false, celli, nodei, nodej);
}
...
{
    return evaluate(&ModelType::dmdt, "dmdt", ModelType::dimDmdt, false, nodei, nodej);
}
...
{
    return evaluate(&ModelType::dmdti, "dmdti", false, celli, nodei, nodej);
}
