...
#include "searchableAnnulusDisk.H"
...
defineTypeNameAndDebug(searchableAnnulusDisk, 0);
addToRunTimeSelectionTable(searchableSurface, searchableAnnulusDisk, dict);
...
Foam::pointIndexHit Foam::searchableAnnulusDisk::findNearest
(
    const point& sample,
...
) const
{
    ...
    // Clip to radius.
    info.setPoint(origin_ + min(max(magV, innerRadius_), outerRadius_)*v);
...
void Foam::searchableAnnulusDisk::findLine
(
    const point& start,
    const point& end,
    pointIndexHit& info
) const
{
    ...
    if (magV >= innerRadius_ && magV <= outerRadius_)
    {
        info.setHit();
        info.setIndex(0);
    }
...
Foam::searchableAnnulusDisk::searchableAnnulusDisk
(
    const IOobject& io,
    const point& origin,
    const point& normal,
    ...
    outerRadius_(outerRadius)
{
    ...
    // vector span(outerRadius_, outerRadius_, outerRadius_);
...
const Foam::wordList& Foam::searchableAnnulusDisk::regions() const
{
    if (regions_.empty())
    {
        regions_.setSize(1);
...
void Foam::searchableAnnulusDisk::boundingSpheres
(
    pointField& centres,
...
void Foam::searchableAnnulusDisk::findNearest
(
...
void Foam::searchableAnnulusDisk::findLineAny
(
    const pointField& start,
    const pointField& end,
    List<pointIndexHit>& info
...
void Foam::searchableAnnulusDisk::findLineAll
(
    const pointField& start,
    const pointField& end,
...
void Foam::searchableAnnulusDisk::getRegion
(
...
void Foam::searchableAnnulusDisk::getNormal
(
    const List<pointIndexHit>& info,
    vectorField& normal
) const
...
void Foam::searchableAnnulusDisk::getVolumeType
(
