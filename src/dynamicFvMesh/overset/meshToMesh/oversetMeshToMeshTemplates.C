...
#include "fvcGrad.H"
#include "oversetDistributedWeightedFvPatchFieldMapper.H"
...
template<class Type>
void Foam::oversetMeshToMesh::add
(
    UList<Type>& fld,
    const label offset
) const
...
void Foam::oversetMeshToMesh::mapSrcToTgt
(
...
) const
{
    ...
    if (singleMeshProc_ == -1)
    {
        ...
        forAll(result, cellI)
        {
            const labelList& srcAddress = tgtToSrcCellAddr_[cellI];
            const scalarList& srcWeight = tgtToSrcCellWght_[cellI];
            const List<point>& srcVec = tgtToSrcCellVec_[cellI];
...
template<class Type, class CombineOp>
Foam::tmp<Foam::Field<Type>> Foam::oversetMeshToMesh::mapSrcToTgt
(
...
template<class Type, class CombineOp>
void Foam::oversetMeshToMesh::mapTgtToSrc
(
...
) const
{
    ...
    if (singleMeshProc_ == -1)
    {
        ...
        forAll(result, cellI)
        {
            const labelList& tgtAddress = srcToTgtCellAddr_[cellI];
            const scalarList& tgtWeight = srcToTgtCellWght_[cellI];
            const List<point>& tgtVec = srcToTgtCellVec_[cellI];
...
template<class Type, class CombineOp>
Foam::tmp<Foam::Field<Type>> Foam::oversetMeshToMesh::mapTgtToSrc
(
...
template<class Type, class CombineOp>
void Foam::oversetMeshToMesh::mapInternalSrcToTgt
(
    ...
    forAll(AMIList, i)
    {
        ...
        (
            ...
            (
                ...
                tgtField.patch(),
                result(),
                oversetDistributedWeightedFvPatchFieldMapper
                (
...
Foam::oversetMeshToMesh::mapSrcToTgt
(
    const GeometricField<Type, fvPatchField, volMesh>& field,
    const CombineOp& cop,
    const bool secondOrder
...
template<class Type, class CombineOp>
void Foam::oversetMeshToMesh::mapInternalTgtToSrc
(
    const GeometricField<Type, fvPatchField, volMesh>& field,
    const CombineOp& cop,
    GeometricField<Type, fvPatchField, volMesh>& result,
...
template<class Type, class CombineOp>
Foam::tmp<Foam::GeometricField<Type, Foam::fvPatchField, Foam::volMesh>>
Foam::oversetMeshToMesh::mapTgtToSrc
(
    const GeometricField<Type, fvPatchField, volMesh>& field,
    const CombineOp& cop,
    const bool secondOrder
