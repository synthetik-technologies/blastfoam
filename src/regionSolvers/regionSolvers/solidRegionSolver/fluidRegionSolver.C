    ...
    velocityFields_(1, "U")
{
    ...
    if (!dynMeshDict.found("diffusivity"))
    {
        ...
        os  << token::END_LIST;
        Info<<mesh_.name()<<" "<<word(os.str())<<endl;
...
bool Foam::regionSolvers::fluid::initialiseMesh(const bool firstIter)
{
    ...
    Info<<"Max mesh velocity = "
        << max(mag(mesh_.phi()/mesh_.magSf())).value() <<endl;
...
bool Foam::regionSolvers::fluid::moveMesh(const bool finalIter)
{
    ...
    if (Pstream::parRun())
    {
        ...
        forAll(meshPoints, i)
        {
            elems[i] = curPoints[meshPoints[i]];
        }
        ...
        forAll(meshPoints, i)
        {
            curPoints[meshPoints[i]] = elems[i];
        }
    }

    const vectorField dpointD(tcurPoints() - mesh_.points());

    mesh_.movePoints(tcurPoints);

    Info<<"Max mesh velocity = "
        << max(mag(mesh_.phi()/mesh_.magSf())).value() <<endl;
    Info<<"Max/Min displacement = "
        << gMaxMagSqr(dpointD) << ", " << gMinMagSqr(dpointD) <<endl;
