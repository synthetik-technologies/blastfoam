...
void Foam::cavitatingFluidBlastThermo<Thermo>::calculate()
{
    ...
    forAll(this->rho_, celli)
    {
        ...
        if (xl < small)
        {
            ...
            const scalar Cpi = tv.Cp(rhoi, ei, Ti);

            this->pRef()[celli] = pi;
            this->CpRef()[celli] = Cpi;
            this->CvRef()[celli] = tv.Cv(rhoi, ei, Ti);
            this->muRef()[celli] = tv.mu(rhoi, ei, Ti);
            this->alphaRef()[celli] = tv.kappa(rhoi, ei, Ti)/Cpi;
        ...
        else if (xv < small)
        {
            ...
            const scalar Cpi = tl.Cp(rhoi, ei, Ti);

            this->pRef()[celli] = pi;
            this->CpRef()[celli] = Cpi;
            this->CvRef()[celli] = tl.Cv(rhoi, ei, Ti);
            this->muRef()[celli] = tl.mu(rhoi, ei, Ti);
            this->alphaRef()[celli] = tl.kappa(rhoi, ei, Ti)/Cpi;
            this->speedOfSoundRef()[celli] =
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        forAll(pT, facei)
        {
            ...
            if (xl < this->residualFac_)
            {
                ...
                palpha[facei] = tv.kappa(rhoi, ei, Ti)/pCp[facei];
                pc[facei] =
                    sqrt(max(tv.cSqr(pi, rhoi, ei, Ti), small));
            }
            else if (xv < this->residualFac_)
            {
                ...
                palpha[facei] = tl.kappa(rhoi, ei, Ti)/pCp[facei];
                pc[facei] =
                    sqrt(max(tl.cSqr(pi, rhoi, ei, Ti), small));
            }
            else
            {
                ...
                pmu[facei] =
                  ...
                  + tl.mu(rhoi, ei, Ti)*xl;
                palpha[facei] =
                    tv.kappa(rhoi, ei, Ti)/pCp[facei]*xv
                  + tl.kappa(rhoi, ei, Ti)/pCp[facei]*xl;
                pc[facei] =
                    sqrt
                    (
                        1.0
