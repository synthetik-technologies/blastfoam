...
:
    MeshObject<fvMesh, UpdateableMeshObject, localLeastSquaresVectors>(mesh),
    local_(local),
    AinvPtr_(nullptr),
...
void Foam::localLeastSquaresVectors::makeLeastSquaresVectors() const
{
    ...
    AinvPtr_ = new tensorField(mesh_.nCells(), Zero);
...
void Foam::localLeastSquaresVectors::updateMesh(const mapPolyMesh&)
{
    deleteDemandDrivenData(AinvPtr_);
    deleteDemandDrivenData(AinvLocalPtr_);
}
