    ...
    deltaAlphaRhoYs_(species_.size())
{
    ...
    if (dict.lookupOrDefault<Switch>("reacting", false))
    {
       chemReader_.set
       (
           chemistryReader<Thermo>::New(thermoDict_, species_).ptr()
       );
        reactions_ = PtrList<Reaction<Thermo>>(chemReader_().reactions());
        speciesComposition_ = chemReader_().specieComposition();
        chemistry_.set
        (
            new ode<StandardChemistryModel<reactionThermo, Thermo>>(*this)
        );
    }
}
...
template<class Thermo>
void Foam::multicomponentFluidThermo<Thermo>::postUpdate()
{
    ...
    if (chemistry_.valid())
    {
        ...
        >(chemistry_()).solve(dT.value());
        forAll(Ys_, i)
        {
            if (active(i))
            {
                Ys_[i].ref() += dT*chemistry_->RR(i)/rho;
                Ys_[i].maxMin(0.0, 1.0);
                Ys_[i].correctBoundaryConditions();
            }
        }
        this->e_ += dT*chemistry_->Qdot()/rho;
        Info<<max(mag(chemistry_->Qdot())).value()<<endl;
    }
}
...
template<class Thermo>
void Foam::multicomponentFluidThermo<Thermo>::update()
{
    updateMixture();
}
...
Foam::tmp<Foam::volScalarField>
Foam::multicomponentFluidThermo<Thermo>::volScalarFieldProperty
(
    const word& psiName,
...
) const
{
    ...
    {
        psi[celli] = (this->volMixture_[celli].*psiMethod)(args[celli] ...);
    }
    ...
    forAll(psiBf, patchi)
    {
        ...
        {
            pPsi[facei] =
                (this->faceMixture_[patchi][facei].*psiMethod)
                (
