...
#include "explicitNonLinGeomUpdatedLagSolid.H"
#include "fvm.H"
#include "fvc.H"
...
defineTypeNameAndDebug(explicitNonLinGeomUpdatedLagSolid, 0);
addToRunTimeSelectionTable
(
    solidModel, explicitNonLinGeomUpdatedLagSolid, dictionary
);
...
explicitNonLinGeomUpdatedLagSolid::explicitNonLinGeomUpdatedLagSolid
(
    dynamicFvMesh& mesh
)
:
...
scalar explicitNonLinGeomUpdatedLagSolid::CoNum() const
{
    // waveSpeed = cellWidth/deltaT
    // So, deltaT = cellWidth/waveVelocity == (1.0/deltaCoeff)/waveSpeed
    // In the current discretisation, information can move two cells per
...
scalar explicitNonLinGeomUpdatedLagSolid::maxCoNum() const
{
    return
