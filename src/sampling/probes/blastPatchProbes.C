...
namespace Foam
{
    defineTypeNameAndDebug(blastPatchProbes, 0);
...
void Foam::blastPatchProbes::findElements
(
    const fvMesh& mesh,
    const bool print,
    const bool movePts
...
bool Foam::blastPatchProbes::write()
{
...
bool Foam::blastPatchProbes::read(const dictionary& dict)
{
    dict.lookup("patchName") >> patchName_;
    return blastProbes::read(dict);
}
