...
) const
{
    ...
    scalar omegaI = omega
    (
        p, rho, T, c, pf, cf, lRef, pr, cr, rRef
    );
...
) const
{
    ...
    const scalar kr = this->kr(kf, p, rho, clippedT, c);
...
) const
{
    ...
    omegaI = omega(p, rho, T, c, pf, cf, lRef, pr, cr, rRef);
    ...
    kfwd = this->kf(p, rho, T, c);
    kbwd = this->kr(kfwd, p, rho, T, c);
...
) const
{
    ...
    scalar dkfdT = this->dkfdT(p, rho, T, c);
    scalar dkrdT = this->dkrdT(p, rho, T, c, dkfdT, kr);
    ...
    scalar dcidT = this->dcidT(p, rho, T, c);
    dcidT *= omegaI;
