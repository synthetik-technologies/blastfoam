...
template<template<class> class PatchField, class Mesh, class CoeffType>
tmp<GeometricField<symmTensor, PatchField, Mesh>>
mechanicalLaw::calcEpsilon
(
    const CoeffType& coeff,
    const GeometricField<symmTensor, PatchField, Mesh>& sigma,
    const GeometricField<symmTensor, PatchField, Mesh>& epsilonP
...
{
    tmp<GeometricField<symmTensor, PatchField, Mesh>> tepsilon
    (
        calcEpsilon(sigma)
    );
    replacePlaneStress(tepsilon.ref(), coeff, sigma, epsilonP);
    return tepsilon;
}
...
) const
{
    ...
    replacePlaneStress(tepsilon.ref(), coeff, sigma);
    return tepsilon;
}
...
{
    return calcEpsilon(coeff(), sigma, epsilonP);
}
...
{
    return calcEpsilon(coeff(), sigma);
}


template<class CoeffType>
tmp<Field<symmTensor>> mechanicalLaw::calcEpsilon
(
    const CoeffType& coeff,
    const Field<symmTensor>& sigma,
    const Field<symmTensor>& epsilonP,
    const label patchi
) const
{
    tmp<Field<symmTensor>> tepsilon(calcEpsilon(patchi));
    replacePlaneStress(tepsilon.ref(), coeff, sigma, epsilonP);
    return tepsilon;
}
...
{
    return calcEpsilon(coeff(), sigma, patchi, epsilonP);
}
...
{
    return calcEpsilon(coeff(), sigma, patchi);
}
