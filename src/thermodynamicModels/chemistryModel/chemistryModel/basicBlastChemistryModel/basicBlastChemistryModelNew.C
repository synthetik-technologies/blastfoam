...
)
{
    ...
    (
        IOobject
        (
            IOobject::groupName("chemistryProperties", thermo.phaseName()),
    ...
    if (!chemistryDict.isDict("chemistryType"))
    {
        FatalErrorInFunction
            ...
            << "    chemistrySolver ode<standardBlastChemistryModel<"
            << "rhoBlastChemistryModel,sutherland<specie<janaf<perfectGas>,"
    ...
    if (cstrIter == thermoConstructorTablePtr_->end())
    {
        ...
        )
        {
            ...
            (
                blastThermo::thermoNameComponents(thermo.thermoName())
            );
        ...
        else
        {
            ...
            thisCmpts.append
            (
                blastThermo::splitThermoName(thermo.thermoName(), 5)
            );
            ...
            forAll(names, i)
            {
                validCmpts.append(blastThermo::splitThermoName(names[i], 7));
            }
