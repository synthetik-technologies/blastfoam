...
typename Table::iterator Foam::fluidThermoModel::lookupThermo
(
    Table* tablePtr,
...
)
{
    ...
    if (cstrIter == tablePtr->end())
    {
        FatalErrorInFunction
            << "Unknown " << fluidThermoModel::typeName << " type " << nl
            << "thermoType " << thermoTypeName << nl << nl
            << "Valid " << fluidThermoModel::typeName << " types are:"
            << nl << nl;
        ...
        forAll(validThermoTypeNames, i)
        {
            wordList names
            (
                fluidThermoModel::splitThermoName(validThermoTypeNames[i])
            );
