...
namespace Foam
{
    defineTypeNameAndDebug(immersedPolyPatch, 0);

    addToRunTimeSelectionTable(polyPatch, immersedPolyPatch, word);
    addToRunTimeSelectionTable(polyPatch, immersedPolyPatch, dictionary);
}

// * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * * * * * //

Foam::immersedPolyPatch::immersedPolyPatch
(
    const word& name,
    const label size,
    const label start,
...
{
    //  immersed is not constraint type so add immersed group explicitly
