...
) const
{
    ...
    scalar w = R.omega(p, this->rhoi_, T, c, pf, cf, lRef, pr, cr, rRef);
    return(w);
}
...
) const
{
    ...
    forAll(reactions_, ri)
    {
        ...
        scalar kfwd, kbwd;
        R.dwdc(p, this->rhoi_, T, c_, J, dcdt, omegaI, kfwd, kbwd, false, dummy);
        R.dwdT(p, this->rhoi_, T, c_, omegaI, kfwd, kbwd, J, false, dummy, nSpecie_);
    }
...
Foam::StandardChemistryModel<ReactionThermo, ThermoType>::tc() const
{
    ...
    if (this->chemistry_)
    {
        forAll(this->thermo().rho(), celli)
        {
            ...
            for (label i=0; i<nSpecie_; i++)
            {
                c_[i] = this->rhoi_*Y_[i][celli]/specieThermo_[i].W();
                cSum += c_[i];
            }
            ...
            forAll(reactions_, i)
            {
                ...
                R.omega(pi, this->rhoi_, Ti, c_, pf, cf, lRef, pr, cr, rRef);
...
) const
{
    ...
    forAll(this->thermo().rho(), celli)
    {
        ...
        for (label i=0; i<nSpecie_; i++)
        {
            ...
            c_[i] = this->rhoi_*Yi/specieThermo_[i].W();
        }

        const Reaction<ThermoType>& R = reactions_[ri];
        const scalar omegai = R.omega(pi, this->rhoi_, Ti, c_, pf, cf, lRef, pr, cr, rRef);
