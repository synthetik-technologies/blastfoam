...
#include "linearTotalDisplacementSolid.H"
#include "fvm.H"
#include "fvc.H"
...
defineTypeNameAndDebug(linearTotalDisplacementSolid, 0);
addToRunTimeSelectionTable(solidModel, linearTotalDisplacementSolid, dictionary);


// * * * * * * * * * * *  Private Member Functions * * * * * * * * * * * * * //


void linearTotalDisplacementSolid::predict()
{
...
:
    ...
    predictor_(solidModelDict().lookupOrDefault("predictor", false))
{
    if (predictor_)
    {
        ...
        if (ddtDScheme == "steadyState")
        {
            FatalErrorInFunction
                ...
                << ") scheme should not be 'steadyState'!" << endl
                << abort(FatalError);
        }
    }
}
